...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let price_provider = PriceProviderFactory::create_provider(
        &bitaxe_clocker::common::CONFIG.get().unwrap().price_provider.provider_type,
    ...
    loop {
        ...
        for bitaxe in &bitaxe_clocker::common::CONFIG.get().unwrap().bitaxes {
            ...
            if let Err(e) = async {
                ...
                let switch_frequency_to: i32 = bitaxe_clocker::bitaxe::should_switch_frequency_to(&client, bitaxe, current_price).await?;
        ...
        logger::info(&format!("Sleeping for {:?} minutes", &bitaxe_clocker::common::CONFIG.get().unwrap().check_interval));
